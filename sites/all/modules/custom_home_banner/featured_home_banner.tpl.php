        ...
        <ul class="flow <?php echo $feature_flag; ?>">
          ...
          foreach ($data as $sub_key => $bod_list) {
            ...
            <li>
              <?php
              global $language;
              $link_to_node = drupal_get_path_alias('node/' . $bod_list["nid"], $bod_list["language"]);
              $curr_site_lang = $language->language;
              $article_lang = $bod_list["language"];
              //echo $curr_site_lang . "--" . $article_lang;

              switch ($article_lang) {
                case 'en':
                  $link_to_node = $base_url . "/" . drupal_get_path_alias('node/' . $bod_list["nid"], $article_lang);
                  //echo $content_link;
                  break;

                default:
                  $link_to_node = $base_url . "/" . $article_lang . "/" . drupal_get_path_alias('node/' . $bod_list["nid"], $article_lang);
                  break;
              }
