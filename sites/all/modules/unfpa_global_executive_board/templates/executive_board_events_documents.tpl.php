...
/**
 * @file
 * Theme implementation to display documents in Exbo events detail page.
 */
?>
<div id='session-documents-wrapper'>
  ...
  <?php endif; ?>
  <?php foreach ($exbo_document_data as $document_data_key => $document_data): ?>

    <?php if (isset($session_title[$document_data_key])) : ?>
      <h3><?php print $session_title[$document_data_key]; ?></h3>
    <?php endif; ?>

    <div class="clearfix documents-wrapper">
      <?php foreach ($document_data as $country_code => $document_data_val): ?>
